...
public class BidsPriceBetaVolWorkflowMain {
   ...
   double[] beta = new double[]{0.09, 0.07, 0.05, 0.1, 0.3, 0.5 };
    //double[] volatility = new double[]{1D,0.9};
  // int[] checkpoint = new int[]{10000000,300,900,1800};
    //int[] checkpoint = new int[]{10000000};
    for(WorkflowPolicies wfPol : values){
		System.out.println(SimProperties.SIM_START_TIME.asDate().getTime());
			for(double a : beta){
			SimProperties.BUFFER_FACTOR_BETA.set(a);
			...
			for (DeadlineBudgetBean oneEntry : readDeadlineBudget) {
				for (int i = 0; i < SimProperties.WF_NUMBER_OF_EXECUTIONS
					...
					try {
						...
						outputResults(out, oneEntry, wfPol.getPolicyName(), i, false, a);
					} catch (NullPointerException e) {
						System.out.println(" Did i can the exception");
						outputResults(out, oneEntry, wfPol.getPolicyName(), i, true,a);
						e.printStackTrace();
					}
    ...
    private static void printHeadersforOutputFile(PrintWriter out) {
		...
		out.print("BID");
		out.print(",");
		out.println("SPOTPRICE");

		
	}

	private static void outputResults(PrintWriter out, DeadlineBudgetBean oneEntry, String policy, int round, boolean exceptionFlag, double vol) {
    	...
    	ArrayList<BidPojo> spotPricePojo = SimulationData.singleton().getStats().getSpotPricePojo();
        Set<BidPojo> s = new TreeSet<BidPojo>(new Comparator<BidPojo>() {

            @Override
            public int compare(BidPojo o1, BidPojo o2) {
                ...
                s.addAll(spotPricePojo);
        
    	ArrayList<BidPojo> newPojolist = new ArrayList<>(s);
    	int counter =0;
    	for(BidPojo bidPojo: newPojolist){
    		out.print(vol);
    		out.print(",");
    		out.print(counter);
    		out.print(",");
    		out.print(bidPojo.getBid());
    		out.print(",");
    		out.println(bidPojo.getSpotPirce());
    		counter++;
    	}
	}
