...
public class ChkptVolWorkflowMain {
   ...
   double[] volatility = new double[]{0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1};
   //double[] volatility = new double[]{SimProperties.CHKPT_VOLATILITY.asDouble()};
   ...
   int[] checkpoint = new int[]{10000000,300,900,1800};
    ...
    for(WorkflowPolicies wfPol : values){
		...
		for(int chkpt : checkpoint){
			SimProperties.FT_CKPT_FREQ.set(chkpt);
			for(double vol : volatility){
			SimProperties.PRICING_TRACE_HISTORY_MOD_FACTOR.set(vol);
			...
			for (DeadlineBudgetBean oneEntry : readDeadlineBudget) {
				...
				for (int i = 0; i < SimProperties.WF_NUMBER_OF_EXECUTIONS
					...
					int upperBound = (int)(90*24*60*vol - (200000/60));
					...
					System.out.println(" Skipped days "+SimProperties.SIM_START_TIME.asDate().getTime());
					...
					try {
						...
						outputResults(out, oneEntry, wfPol.getPolicyName(), i, false, chkpt, vol);
					} catch (NullPointerException e) {
						System.out.println(" Did i can the exception");
						outputResults(out, oneEntry, wfPol.getPolicyName(), i, true, chkpt, vol);
						e.printStackTrace();
					}
	...
	private static void outputResults(PrintWriter out, DeadlineBudgetBean oneEntry, String policy, int round, boolean exceptionFlag, int chptFreq, double vol) {
