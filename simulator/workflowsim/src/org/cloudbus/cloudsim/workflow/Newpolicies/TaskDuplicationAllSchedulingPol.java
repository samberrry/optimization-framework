...
public class TaskDuplicationAllSchedulingPol implements WorkflowSchedulingPolicy {

	private static final String NAME = "TaskDuplicationAllSchPol";
    ...
    public TaskDuplicationAllSchedulingPol() {
...
//				HESSAM CHANGED THIS LINE OF CODE
//				dec = new SchedulingDecision(task, false, Region.getDefault(), AZ.ANY, cheapestInstance, preferedOS, null, true, currentTime,
		...
		// Making sure that the replication does not happen on the same resource
		// as the other replica
		// We replicates for all those who use OD and are past LTO
		if (task.getJob().getNumberOfActiveTasks() <= 1) {
			Collection<Resource> unusedInstance = new LinkedHashSet<Resource>(
					myInstances);
