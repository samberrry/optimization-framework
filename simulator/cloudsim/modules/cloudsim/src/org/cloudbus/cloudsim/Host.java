...
/**
 * Host class extends a Machine to include other hostList beside PEs to support
 * simulation of virtualized grids. It executes actions related to management of
...
public class Host {
    ...
    public Host(int id, int pes, int ram, double peMips, boolean timeShared) {
	this(id, pes, ram, peMips);
	if (timeShared) {
	    setVmScheduler(new VmSchedulerTimeShared(this.peList));
	}
    }

    public Host(int id, int pes, int ram, double peMips) {
	setId(id);
	setRamProvisioner(new RamProvisionerSimple(ram));
	setBwProvisioner(new BwProvisionerSimple(Long.MAX_VALUE));
	setStorage(Long.MAX_VALUE);

	List<Pe> peL = new ArrayList<Pe>();
	for (int i = 0; i < pes; i++) {
	    peL.add(new Pe(i, new PeProvisionerSimple(peMips)));
	}
	setPeList(peL);
	setVmScheduler(new VmSchedulerSpaceShared(this.peList));
	setVmList(new ArrayList<Vm>());
	setFailed(false);
    }
	...
	getVmList().add(vm);
	vm.setCurrentAllocatedMips(getVmScheduler().getAllocatedMipsForVm(vm));
	vm.setHost(this);
	return true;
    }
	...
	builder.append("Host [id=").append(this.id).append("]");
	...
	throw new IllegalArgumentException("VM does not exist: " + vmId);
    }
