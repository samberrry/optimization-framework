...
public class FileAttribute {
	...
	this.ownerName = null;
	this.id = File.NOT_REGISTERED;
	this.checksum = 0;
	this.type = File.TYPE_UNKOWN;
	...
	attr.setFileSize(this.size);
	attr.setResourceID(this.resourceId);
	attr.setOwnerName(this.ownerName);
	attr.setUpdateTime(this.lastUpdateTime);
	attr.setRegistrationId(this.id);
	attr.setType(this.type);
	attr.setChecksum(this.checksum);
	attr.setCost(this.cost);
	attr.setMasterCopy(this.masterCopy);
	attr.setReadOnly(this.readOnly);
	attr.setName(this.name);
	attr.setCreationTime(this.creationTime);
	...
	int length = DataCloudConstants.PKT_SIZE;
     ...
     * {@link gridsim.CloudSim#minTime()} method.
     * 
     * @param time
	...
	if (time <= 0 || time < this.lastUpdateTime) {
	    return false;
	}
	...
	boolean result = true;
	if (this.id == File.NOT_REGISTERED) {
	    result = false;
	}
