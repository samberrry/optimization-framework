...
public class PeList {
     ...
     * @post $none
     */
    public static <T extends Pe> double getMips(List<Pe> peList, int id) {
     ...
     * @post $none
     */
    public static <T extends Pe> double getTotalMips(List<Pe> peList) {
	double totalMips = 0;
	for (Pe pe : peList) {
     ...
     * @param failed
     *        the new value for the "failed" parameter
     */
    public static <T extends Pe> void setStatusFailed(List<Pe> peList, String resName, int hostId,
	    boolean failed) {
	String status = null;
	if (failed) {
	    status = "FAILED";
	...
	Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + (resName + " - Machine: " + hostId + " is " + status));
