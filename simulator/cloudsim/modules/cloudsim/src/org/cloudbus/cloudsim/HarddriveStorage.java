...
/**
 * An implementation of a storage system. It simulates the behaviour of a
 * typical harddrive storage. The default values for this storage are those of
 * a Maxtor DiamonMax 10 ATA harddisk with the following parameters:
 * <ul>
 * <li> latency = 4.17 ms
...
public class HarddriveStorage implements Storage {
	...
	public File getFile(String fileName) {
        ...
        if (fileName == null || fileName.length() == 0) {
            Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + (name + ".getFile(): Warning - invalid "
	    + "file name."));
            return obj;
        }
    ...
    private boolean isFileValid(File file, String methodName) {
        ...
        if (file == null) {
            Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + (name + "." + methodName
	...
	public double addFile(File file) {
        ...
        if (file.getSize() + currentSize > capacity) {
            Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + (name + ".addFile(): Warning - not enough space"
	...
	public double addFile(List<File> list) {
        ...
        if (list == null || list.isEmpty()) {
            Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + (name + ".addFile(): Warning - list is empty."));
            return result;
        }
	...
	public boolean contains(String fileName) {
        boolean result = false;
        if (fileName == null || fileName.length() == 0) {
            Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + (name
