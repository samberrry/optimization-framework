...
public class CloudSimShutdown extends SimEntity {
     ...
     * <p>
     * The total number of grid user entity plays an important role to determine
     * whether all hostList should be shut down or not. If one or more users are
     * still not finish, then the hostList will not be shut down. Therefore, it
     * is important to give a correct number of total grid user entity.
    ...
    /**
     * The main method that shuts down hostList and Cloud Information Service
     * (GIS). In addition, this method writes down a report at the end of a
     * simulation based on <tt>reportWriterName</tt> defined in the Constructor. <br>
     * <b>NOTE:</b> This method shuts down grid hostList and GIS entities either
     * <tt>AFTER</tt> all grid users have been shut down or an entity requires
     * an abrupt end of the whole simulation. In the first case, the number of
     * grid users given in the Constructor <tt>must</tt> be correct. Otherwise,
     * CloudSim package hangs forever or it does not terminate properly.
     * 
     * @param ev
     *        the ev
     * @pre $none
	...
	this.numUser--;
	if (this.numUser == 0 || CloudSimTags.ABRUPT_END_OF_SIMULATION.equals(ev.getTag())) {
