...
public class NetworkTopology {
	...
	public static void buildNetworkTopology(String fileName) {
    	Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + ("Topology file: " + fileName));
		...
		try{
			...
			Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + ("Problem in processing BRITE file. Network simulation is disabled. Error: "+e.getMessage()));
		}
			...
			try{
				if(!map.containsKey(cloudSimEntityID)){ //this CloudSim entity was already mapped?
					if(!map.containsValue(briteID)){ //this BRITE node was already mapped?
						...
						Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + ("Error in network mapping. BRITE node "+briteID+" already in use."));
					}
				} else {
					Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + ("Error in network mapping. CloudSim entity "+cloudSimEntityID+" already mapped."));
				}
			} catch (Exception e){
				Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + ("Error in network mapping. CloudSim node "+cloudSimEntityID+" not mapped to BRITE node "+briteID+"."));
			}
		}
	}
	...
	public static void unmapNode(int cloudSimEntityID){
		if(networkEnabled){
			try{
				...
				Log.logger.info("[" + Double.toString(CloudSim.clock()) + "] " + ("Error in network unmapping. CloudSim node: "+cloudSimEntityID));
			}
		}
	}
