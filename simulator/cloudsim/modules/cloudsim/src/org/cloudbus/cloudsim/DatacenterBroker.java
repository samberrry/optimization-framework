...
public class DatacenterBroker extends SimEntity {
     ...
     * 
     * @param name
     *        name to be associated with this entity (as required by Sim_entity
     ...
     * 
     * @param cloudletId
     *        ID of the cloudlet being bound to a vm
     * @param vmId
     *        the vm id
     * 
     * @pre cloudletId > 0
	...
	if (getVmsCreatedList().size() == getVmList().size() - getVmsDestroyed()) { // all
										    // the
										    // requested
										    // VMs
										    // have
	    ...
	    if (getVmsRequested() == getVmsAcks()) { // all the acks received,
						     // but some VMs were not
						     // created
	...
	if (getCloudletList().size() == 0 && this.cloudletsSubmitted == 0) { // all
									     // cloudlets
									     // executed
	    Log.logger.info("["
		    + Double.toString(CloudSim.clock())
