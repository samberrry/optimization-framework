...
public class CSVWriter {
    ...
    public static void processResults(ArrayList<RunResult> runResults, int budgeListSize){
        ...
        for (int i = 0; i < budgeListSize; i++) {
            ...
            cSum = 0 , mSum = 0 , fSum = 0, mMax = 0, mMin = 9999999999.9;
            long sec = -1;
            ...
            for (int j = 0; j < runResults.size(); j++) {
               ...
               if (mMax < runResults.get(j).solutionArrayListToCSV.get(i).makespan){
               ...
               if (mMin > runResults.get(j).solutionArrayListToCSV.get(i).makespan){
